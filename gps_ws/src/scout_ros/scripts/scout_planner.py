...
class gen_planner():
    def __init__(self):
        ...
        ctrl_pub = rospy.Publisher('/cmd_vel',Twist, queue_size=1) ## Vehicle Control
        ctrl_msg= Twist()
        ...
        #subscriber
        rospy.Subscriber("/Object_topic", ObjectStatusList, self.objectInfoCB) # 주변 물체 정보 subscribe하면 저장
        rospy.Subscriber("/gps", GPSMessage, self.gpsCB) # gps값 subscribe하면 저장
        self.image_sub = rospy.Subscriber("/imu", Imu, self.imuCB) # imu값 subscribe하면 euler값으로 변환 후 저장
        self.ego_sub = rospy.Subscriber("/scout_status",ScoutStatus, self.statusCB) # scout_mini 속도 subscribe하면 저장

        #def
        # 초기상태, 각 상태 subscribe하면 True 전환
        ...
        #class
        # utils.py에서 import
        pure_pursuit=purePursuit() ## purePursuit import, 객체 생성
        self.cc=cruiseControl(0.5,1.0) ## cruiseControl import (object_vel_gain, object_dis_gain), 속도 gain, error gain, 객체 생성
        self.vo=vaildObject() ## 장애물 유무 확인 (TrafficLight), 객체 생성
        self.proj_UTM = pyproj.Proj(proj='utm', zone=52, ellps='WGS84', preserve_units=False)

        self.global_path = path_reader.read_txt(self.path_name) # test1.txt 읽음

        normal_velocity = 11/3.6
        vel_planner=velocityPlanning(normal_velocity,0.15) ## 속도 계획, (최대 속도, 도로 마찰계수), 객체 생성
        vel_profile=vel_planner.curveBasedVelocity(self.global_path,100) # test1.txt에 기록된 위치, 
